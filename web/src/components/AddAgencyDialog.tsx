...
export default function AddAgencyDialog({ open, editFor, onClose, onSaved }: Props) {
  ...
  // ── GPS pinning ──────────────────────────────────────────────────────────
  const pinLocation = () => {
    if (!navigator.geolocation) { setError('Browser does not support Geolocation'); return; }
    setGpsLoading(true);
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setError('Unable to retrieve GPS location'); setGpsLoading(false); },
    );
  };
  ...
  // ── Submit ────────────────────────────────────────────────────────────────
  const save = async () => {
    if (!form.name.trim()) { setError('Please enter the Agency name'); return; }
    if (!form.ownerName.trim()) { setError('Please enter the contact person name'); return; }
    if (!form.phone.trim()) { setError('Please enter the phone number'); return; }
    setSaving(true);
    setError('');
    ...
    try {
      if (editFor) {
        ...
        setSnack('Agency saved successfully');
      } else {
        await api.post('/agencies', payload);
        setSnack('Agency created successfully');
      }
      onSaved();
      onClose();
    } catch (e) {
      ...
      >
        ...
        <DialogContent dividers>
          <Stack spacing={1} mt={0.5}>
            ...
            {dupHits.length > 0 && (
              <Alert severity="warning" icon={<WarningAmberIcon />}>
                <Typography variant="body2" fontWeight={600}>
                  Duplicate record found ({dupHits.length} item{dupHits.length > 1 ? 's' : ''}):
                </Typography>
                {dupHits.map((h) => (
                ...
                <Typography variant="caption" color="text.secondary">
                  You can still proceed to create
                </Typography>
              </Alert>
            )}

            {/* ── SECTION 1: General Information ── */}
            <Section title="1. General Information" defaultExpanded>
              ...
              <TextField
                ...
                disabled
                size="small"
                helperText="Code is auto-generated by the system"
              />
            ...
            <Section title="2. Agreement Information">
              <FormControlLabel
                control={
                  <Switch
              ...
              {form.agreementActive &&
                ...
                agreementDaysLeft <= 90 && (
                  <Alert severity="warning">
                    {agreementDaysLeft <= 0
                      ? `Agreement expired ${Math.abs(agreementDaysLeft)} day(s) ago`
                      : `Agreement expires in ${agreementDaysLeft} day(s)`}
                  </Alert>
                )}
            </Section>

            {/* ── SECTION 3: Sales Performance ── */}
            <Section title="3. Sales Performance">
              <FormControlLabel
                control={
                  <Switch
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                />
                <Typography variant="caption" color="text.secondary">
                  (A≥50, B≥20, C≥1, D=0 units)
                </Typography>
              </Stack>
            </Section>

            {/* ── SECTION 4: Office Information ── */}
            <Section title="4. Office Information">
              <FormControlLabel
                control={
                  <Switch
            ...
            <Section title="5. Marketing Information">
              <TextField
            ...
            <Section title="7. Agency Management">
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <TextField
                  select
                  label="Agency Level"
                ...
                >
                  <MenuItem value="">—</MenuItem>
                  <MenuItem value="1">1x / month</MenuItem>
                  <MenuItem value="2">2x / month</MenuItem>
                  <MenuItem value="4">4x / month</MenuItem>
                </TextField>
              </Stack>
        ...
        <DialogActions>
          ...
          >
            {editFor ? t('common.save') : 'Create Agency'}
          </Button>
        </DialogActions>
      </Dialog>
